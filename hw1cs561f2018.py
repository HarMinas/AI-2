...
def maxVal(offCount, array):
    ...
    maximum = 0
    for i in array:
        validMoves = getValidMoves(array, i)
        valMovLen = len(validMoves)
        if offCount > valMovLen:
            continue
        currVal = i.value + maxVal(offCount, validMoves)
        if currVal > maximum:
            maximum = currVal
...
output = maxVal(offLeft, sortedList)
print(output)
